...
ENDDATE_ANCHOR = "Omit when no end date is recorded; null is invalid. Use status for adjustment state."
...
def rows_for(name, schema, meta):
    ...
    def add(prefix, p, r, s, depth=0, parent_priv=None):
        ...
        if name == "ReasonableAdjustment" and p == "endDate" and depth == 0: anchor = ENDDATE_ANCHOR        # the carried v0.1 anchor said null means a standing adjustment; the schema forbids null
...
def compose():
    ...
    s2 = s2.replace("![OWHS v0.1 entity-relationship diagram](../site/owhs-erd-v0.1.svg)", "![OWHS entity-relationship diagram, drawn for v0.1 and unchanged in v0.2](../site/owhs-erd-current.svg)")
...
def self_test():
    ...
    ra = [ln for ln in text[text.index("### ReasonableAdjustment"):text.index("### ", text.index("### ReasonableAdjustment") + 5)].splitlines() if ln.startswith("| `endDate` |")]
    t("ReasonableAdjustment.endDate carries the corrected anchor, keeps individual-employer, and no other endDate anchor changed", len(ra) == 1 and ENDDATE_ANCHOR in ra[0] and "individual-employer" in ra[0] and "standing adjustment if null" not in text and "../site/owhs-erd-current.svg" in text)
...
PUBLISH_RULES = [
    ...
    ("![OWHS entity-relationship diagram, drawn for v0.1 and unchanged in v0.2](../site/owhs-erd-current.svg)", "![OWHS entity-relationship diagram, drawn for v0.1 and unchanged in v0.2](../owhs-erd-current.svg)"),
