...
`tools/measure_recall.py` measures held-out known-item retrieval against a judged bibliography: which judged-relevant holdout works the frozen discovery queries returned without being given their identifiers. Coverage (which indexes contain a work, looked up by identifier) is a separate quantity reported beside it. Before any arithmetic the evaluator checks the contracts: canonical identities, links naming real instruments and properties, judged links with a rationale and a read source, one component per work, study families not crossing the split, exposure propagated to a whole component, the declared deterministic split algorithm reproduced, the candidate artefact bound to the locked query file and window with recognised discovery routes, and coverage rows with provenance from the configured providers only. Seeds are derived from the pinned query file; enabled sources from its provider profile. Seeds, disclosed calibration works and works previously exposed to query tuning are excluded and counted; unresolved relevance or boundary dates make the figure provisional and are never dropped from a denominator; a zero denominator is null and not evaluated, never 100 per cent. Every miss is listed. The judged manifest is private until the evaluation has run and a neutral report has been reviewed; `evidence/evaluation-tests/` holds the public manifest schema and synthetic fixtures. No recall figure exists yet.
