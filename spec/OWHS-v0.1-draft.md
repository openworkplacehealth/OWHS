...
Every list is a standalone JSON file with its own `version` (semver), independent of the spec version, registered in [`_registry.json`](codelists/_registry.json). Schemas pin a list as `name@version`, so a code list can add values (minor bump) without a spec release. Files: [codelists/](codelists/).
