...
def md_to_html(text):
    ...
    def inline(t):
        ...
        t = re.sub(r"doi:(10\.(?:\([^()\s]*\)|[^\s(),])+?)(?=[),\s])", r'<a href="https://doi.org/\1">doi:\1</a>', t)
...
def self_test():
    """Link conversion keeps a URL whole when it contains balanced parentheses (DOIs such as 10.1016/s0140-6736(16)31279-x)
    and still stops at the closing parenthesis of the citation, a comma or whitespace; a URL without parentheses is unchanged."""
    cases = [
        (md, "change ([West C. 2016](https://doi.org/10.1016/s0140-6736(16)31279-x)). However",
         'change (<a href="https://doi.org/10.1016/s0140-6736(16)31279-x">West C. 2016</a>). However'),
        (md, "see [a](https://example.org/p) and [b](https://example.org/q).",
         'see <a href="https://example.org/p">a</a> and <a href="https://example.org/q">b</a>.'),
        (md_to_html, "text ([Bastien 2001](https://doi.org/10.1016/s1389-9457(00)00065-4)). Next",
         '<p>text (<a href="https://doi.org/10.1016/s1389-9457(00)00065-4">Bastien 2001</a>). Next</p>'),
        (md_to_html, "(doi:10.1016/s1389-9457(00)00065-4, and doi:10.1002/mpr.145)",
         '<p>(<a href="https://doi.org/10.1016/s1389-9457(00)00065-4">doi:10.1016/s1389-9457(00)00065-4</a>, and <a href="https://doi.org/10.1002/mpr.145">doi:10.1002/mpr.145</a>)</p>'),
    ]
    failures = []
    for fn, given, want in cases:
        got = fn(given)
        if got != want:
            failures.append(f"{fn.__name__}({given!r})\n  got  {got!r}\n  want {want!r}")
    if failures:
        print("self-test FAILED"); print("\n".join(failures)); sys.exit(1)
    print(f"self-test ok: {len(cases)} link-conversion cases (parenthesised DOIs whole, plain links unchanged)")
