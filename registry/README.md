...
| `registry_gate.py` | The conformance gate: every rule the rubric states that a machine can check. The build refuses to run if the dataset breaks one. |
...
The generator emits no robots posture of its own; `tools/stamp_canonical.py` applies the one word in
`robots-policy.txt` to every page afterwards, so a rebuild cannot change whether the site is indexed.
...
committed one (the PDF is excluded: its bytes carry the build date). The CI workflow runs this on every
push and pull request, so a change to the dataset, the rubric or the generator cannot merge without the
pages that show it.

## What the build refuses

The gate fails on any dataset rule the rubric states and the data breaks. The build also fails if any
