# Instrument registry grading rubric, version 1.1

*Open Workplace Health Standard, instrument registry. Rubric v1.1, 2 September 2026, in force for registry dataset v0.4.0 onward. Licence CC BY 4.0.*

**Version 1.1, 2 September 2026, before first publication.** Change from 1.0: indirectness is defined by population type, not by country. Cells that had been marked indirect only because the evidence was earned outside the United Kingdom, on working-adult samples, were re-rated; the reason recorded on each cell was the basis. No other rule changed. The re-rate is logged as correction C-0004 in the dataset and every cell now records the reason for its indirectness flag in the flag itself.
...
**Honesty note on provenance.** The grades in dataset v0.3.0 were assigned in July 2026 (passes one and two) by one rater working to the rules in the research prompts and schema v0.2, before this document existed. Version 1.0 codified that practice as it was applied; it did not introduce new thresholds, and no grade was revisited to fit it. Version 1.1 changed one rule (section 6) and re-rated the cells that rule touched, before the registry was first published, so that a reader never meets a grade that had been held down for the country its evidence came from. Where the practice was looser than the text below, the text says so. The point of publishing it is to make the next grader's work reproducible and to give reviewers something concrete to tear apart.
...
| `indirectness` | `direct` or `indirect` relative to working adults, followed by the reason (section 6); the grade is already downgraded for it. |
...
| `confidence_note` | Why the grade sits where it does: what would move it up, what holds it down. Not yet populated as a separate field (section 12); in dataset v0.4.0 the reason travels in the `indirectness` string and the findings. |
...
Every grade carries `direct` (working adults, or a working-age general population close enough that the rater does not expect the property to differ) or `indirect` (evidence earned in clinical, student, adolescent, older-adult or otherwise non-working samples, or in settings far from a workplace). An indirect cell has already been downgraded for it, usually by one step, occasionally two where the setting is far from a workplace (inpatient samples, children). **Country and language are never a reason for indirectness.** Where the evidence was earned is recorded in the findings and under populations, languages and norms; whether scores mean the same thing across countries and languages is graded under measurement invariance. A record-level `deployment_context_caveat` states once anything that cuts across every property, such as a clinical-origin instrument deployed in a workplace, and is inherited by every cell.

The flag is written as the word followed by the reason: `direct; occupational samples in five countries` or `indirect; clinical and student samples only`. A cell recorded as Absent or Not-applicable carries the flag too, so that a reader can see whether the absence is population-general (nothing located in any population) or a category error.

**Populations, languages and norms.** The ninth property grades the breadth and quality of the population evidence: how many working populations, languages and settings the instrument has been examined in, and whether reference norms exist and are openly checkable. Norms are recorded by country as a fact. The absence of norms for any particular country is never a downgrade.
...
In dataset v0.4.0 every cell is `assessed`, `assessed_absent` or `not_applicable`. The registry treats a page that lets `assessed_absent` and `not_assessed` blur as a defect, because it misleads exactly the reader it exists to protect.
...
4. **Grade.** Start from the best available study quality; downgrade for inconsistency, imprecision (small or few samples) and indirectness (section 6); assign status (section 4) and evidence form (section 5). Record the reason for the indirectness flag in the flag.
...
## 12. Known limits of version 1.1
...
- The re-rate under section 6 was made by the same single rater from the reasons recorded on each cell, without a fresh literature search; `as_of` on those cells is unchanged and `grade_last_confirmed` is the re-rate date.
- `confidence_note` is not yet populated as a separate field; the reason for each flag sits in the `indirectness` string and the findings carry the numbers.
